...
#DIMS: 1080, 1920
image_choices = ['abstract_bwlines.jpeg','marfa.png','mountain_aerial.jpg','unit1.jpg','woman.png','pointillism.png']

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument('-wp', '--write_path', type=str, help='Write path for video',
                        required=True)
#    parser.add_argument('-ip', '--image_path', type=str, help='Image path for video',
#                        required=True)
...
image_path = os.path.join('/home/jordan/photos/', im_choice)
...
if mod.lower() in ['pixelate', 'pixel']:
    modify = 'pixel'
    box_shape = int(input(f"What size pixelation would you like? Note the dimensions of this image are {height}, {width}. "))
...
while True:
    ...
    if current_time - last_read >= read_interval:
        image_path = os.path.join('/home/jordan/photos', random.choice(image_choices))
